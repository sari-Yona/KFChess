...
public class MoveTableListener implements EventListener<PieceMovedEvent> {
    ...
    private int getPieceValue(String pieceType) {
        ...
        switch (pieceType.toUpperCase()) {
            case "P": return 1;  // Pawn (חייל)
            case "N": return 3;  // Knight (פרש)
            case "B": return 3;  // Bishop (רץ)
            case "R": return 5;  // Rook (צריח)
            case "Q": return 9;  // Queen (מלכה)
            case "K": return 0;  // King (no score for capturing king - game ends)
            default: return 0;
        }
    }
    ...
    public JTable getWhiteTable() { return whiteTable; }
    public JTable getBlackTable() { return blackTable; }
    public JLabel getWhiteScoreLabel() { return whiteScoreLabel; }
    public JLabel getBlackScoreLabel() { return blackScoreLabel; }
